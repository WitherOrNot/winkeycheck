...
def query_key(pkey, pl_data, pkc, config_ext="Retail"):
    ...
    if data.find("./{*}Body/{*}Fault") is None:
        return data.find("./{*}Body/{*}RequestSecurityTokenResponse/{*}RequestedSecurityToken/{*}TmsResponseToken/{*}Values/{*}TokenEntry/{*}Value").text, "", True
    else:
        return data.find("./{*}Body/{*}Fault/{*}detail/{*}HRESULT").text, data.find("./{*}Body/{*}Fault/{*}detail/{*}Messages/{*}Message").text, False

if __name__ == "__main__":
    p = argparse.ArgumentParser(description="Product key checker. Please note this tool will consume one activation per product key checked.")
